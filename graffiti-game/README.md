...
Third person while you explore, first person the moment you start painting.
...
| <kbd>Space</kbd> | Jump |
| <kbd>V</kbd> | Switch first / third person |
| <kbd>P</kbd> | Paint the wall you are near (within 5 m) |
| <kbd>E</kbd> | Interact / **hold** to finish a piece |
| <kbd>F</kbd> | Raise / lower the camera |
| <kbd>G</kbd> | Open your flicks |
| <kbd>Esc</kbd> | Pause |

**In paint mode the movement keys are rebound**, because the mouse is the can:

| Key | Action |
| --- | --- |
| Drag the mouse | Spray — the pointer *is* the can |
| <kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> | Aim (swings the view) |
| <kbd>Shift</kbd> + WASD | Move without leaving paint mode |
| <kbd>C</kbd> / wheel | Next colour |
| <kbd>1</kbd>–<kbd>9</kbd> | Pick a can directly |
...
| <kbd>E</kbd> | Hold to finish the piece |
...
**Paint mode releases pointer lock and gives the mouse absolute position.**
The pointer becomes a real drawing cursor that drags across the wall exactly
where you put it. This is the single biggest thing separating painting that
feels like painting from painting that feels like wrestling a camera: a
relative-delta virtual cursor drifts, and you lose the ability to place a line
where you meant to. WASD takes over aiming so the mouse is free, and
<kbd>Shift</kbd> + WASD repositions you mid-piece without leaving the mode.

**Nearly every wall is paintable.** A dozen named spots are hand-placed with
deliberate risk ratings (those are the ones the objectives talk about); the
rest are generated by tiling panels along every facade, pillar and hoarding
that borders somewhere you can stand. Panels are invisible and carry no canvas
until paint actually lands on them, so ~60 paintable surfaces cost nothing at
boot — the texture is allocated on the first stroke.

**Only painting draws attention.** Walking, running and loitering are not
crimes: witnesses build suspicion solely while you are actually spraying, and
police escalate only once there is something to answer for. Getting seen is a
consequence of what you do at a wall, not of existing near one.
